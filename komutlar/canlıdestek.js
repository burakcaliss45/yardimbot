...
const emoji7 = message.client.emojis.get('📞');
    ...
    client.channels.get(destekKanal).send('📞 Destek çagrısı bağlanmak için `katıl` yazınız. ☎️ İptal Etmek İçin `kapat` yazınız.')
    ...
    collector.on('end', (collected, reason) => {
      ...
      if (reason === 'aborted') {
        message.reply('☎️Çağrı Destek Ekibinden reddedildi.')
      ...
      if (reason === 'success') {
        ...
        client.channels.get(destekKanal).send('☎️ Destek çağrısını kapatmak için `kapat` yazınız.')
        chan.send(`${message.author}`)
        chan.send('📡Çağrınız bir destek yetkili tarafından alındı!📡')
        chan.send('⏳En Yakın Zamanda Size Yardımcı Olacagız.⏳')
        chan.send('☎️Destek çagrısı kapatmak için `kapat` yazınız.☎️')
        isEnabled = true
        client.on('message', message => {
          function contact() {
            ...
            if (message.content.startsWith('kapat')) {
              message.channel.send('☎️ Çağrı Kapatıldı.')
              if (message.channel.id === chan.id) client.channels.get(destekKanal).send('☎️ Çağrı karşı taraftan kapatıldı.')
              if (message.channel.id === destekKanal) chan.send('☎️ Çağrı karşı taraftan kapatıldı.')
